...
import edu.pitt.medschool.model.TSData.HeaderMapping;
import edu.pitt.medschool.model.TSData.HeaderReverseMapping;
...
public class HeaderLookupServiceDemo implements CommandLineRunner {
    ...
    private List<HeaderReverseMapping> headerTypeToPossibleFullSidList(@Nullable String type, @Nullable String note) {
        ...
        List<HeaderReverseMapping> c = jdbcTemplate.query(String.format("SELECT type, SID, MAX(SID_Count) AS B FROM `%s` WHERE `type` LIKE ? OR 'note` LIKE ? GROUP BY SID", DBConfiguration.RelationalData.HEADERMAPING), new Object[] { type, note }, (rs, rowNum) -> new HeaderReverseMapping(rs.getString("type"), rs.getString("SID"), rs.getInt("B")));
    ...
    private HeaderMapping headerSidToDetail(String sidPrefix) {
        ...
        List<HeaderMapping> c = jdbcTemplate.query(String.format("SELECT * FROM `%s` WHERE SID = ?", DBConfiguration.RelationalData.HEADERMAPING), new Object[] { sidPrefix }, (rs, rowNum) -> new HeaderMapping(rs.getLong("id"), rs.getString("type"), rs.getString("electrode"), rs.getDouble("freq_low"), rs.getDouble("freq_high"), rs.getString("notes"), rs.getString("SID"), rs.getInt("SID_Count")));

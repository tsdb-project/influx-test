...
public class PerformanceTest {
	...
	public static void columnTest(int limit) {
		...
		coloums.append(") / " + count
				+ " as avg from \"PUH-2018-026\" where arType = 'ar' limit 3600) group by time(1m) fill(none)");
	...
	public static void downsampleTest(int limit) {
		...
		String last = String.format(
				" as avg from \"PUH-2018-026\" where arType = 'ar' limit %s) group by time(%sm) fill(none)",
				3600 * limit, limit);
		coloums.append(last);
	...
	public static void colGroupRowTest(int colLimit, int rowLimit) {
		...
		coloums.append(String.join(", ", colList));
		String last = String.format(" from \"PUH-2010-087\" where arType = 'ar' group by time(%sm) fill(none) limit %s",
				1, 60 * rowLimit);
		coloums.append(last);
		...
		idb.query(new Query(coloums.toString(), "data"));
		long end = System.currentTimeMillis();
		System.out.println(String.format("%s,%s,%s,%s,%s", colLimit, colLimit * colLimit, rowLimit, rowLimit * rowLimit,
				end - start));
	}
...
;
					...
					String firstRecordTime = firstRecordResult.getResults().get(0).getSeries().get(0).getValues().get(0)
							.get(0).toString();
					QueryResult lastRecordResult = idb.query(new Query(lastRecordTimeQuery, dbName));
					String lastRecordTime = lastRecordResult.getResults().get(0).getSeries().get(0).getValues().get(0)
							.get(0).toString();

					int offset = Integer.valueOf(firstRecordTime.substring(17, 19));

					String queryString = String.format(template, allColumns, patientId, firstRecordTime, lastRecordTime, offset);
					...
					System.out.println(
							patientId + ": " + result.getResults().get(0).getSeries().get(0).getValues().size());
				} catch (Exception e) {
					e.printStackTrace();

...
(function (global, factory) {
    ...
    var DiscreteLegend = Kapsule({
        ...
        update: function update(state) {
            ...
            state.el.selectAll('.color-slot')
                .on("click",function(s){
                    state.clickFunction && state.clickFunction(s);
                })
        }
    });
    ...
    var legend = Kapsule({
        ...
        update: function update(state) {
            ...
            DiscreteLegend ().width(state.width).height(state.height).scale(state.scale).label(state.label).clickFunction(state.clickFunction)(state.legend.node());
        }
    });
    ...
    var timelines = Kapsule({
        props: {
            ...
            onLegendClick: {}// When user clicks on the legend, Returns (legend object) respectively
        },
        methods: {
        ...
        init: function init(el, state) {
            ...
            function addTooltips() {
                ...
                state.segmentTooltip = d3Tip().attr('class', 'chart-tooltip segment-tooltip').direction('s').offset([5, 0]).html(function (d) {
                    ...
                    return '<strong>' + d.labelVal + ' </strong>';
                });
        ...
        update: function update(state) {
            ...
            function adjustLegend() {
                ...
                state.colorLegend.width(Math.max(120, state.graphW / 3 * (state.zQualitative ? 2 : 1))).height(state.topMargin * .6).scale(state.zColorScale).label(state.zScaleLabel).clickFunction(state.onLegendClick);
         ...
         */
            ...
            function renderTimelines(maxElems) {
                ...
                newSegments.on('mouseover', function () {
                    ...
                    }).attr('height', state.lineHeight).style('fill-opacity', .6);
                }).on('click', function (s) {
